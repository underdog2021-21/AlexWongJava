...
public class GreenImageUploadScan {
     ...
     * 调用阿里云接口检测 图片
     * @param imageByteList
     * @return  map  key-suggestion  val -  pass：结果正常，无需进行其余操作。
     *                                      review：结果不确定，需要进行人工审核。
     *                                      block：结果违规，建议直接删除或者限制公开。
     *              key -label       val - 错误原因
     * @throws Exception
     */
    public Map<String,String> imageScan(List<byte[]> imageByteList) throws Exception {
        ...
        List<JSONObject> checkImgUrlList = new ArrayList<>();
...
//            使用工具上传图片
            ...
            checkImgUrlList.add(task);
        }
        httpBody.put("tasks", checkImgUrlList);
