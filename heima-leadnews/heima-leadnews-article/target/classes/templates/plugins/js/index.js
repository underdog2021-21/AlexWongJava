...
new Vue({
  ...
  data() {
    return {
      // Minio模板应该写真实接口地址
      baseUrl: 'http://localhost:8801/app', //'http://172.16.17.191:5001',
      token: '',
      equipmentId: '',
      articleId: '',
      title: '',
  ...
  methods: {
    ...
    async handleClickArticleUnlike() {
      const url = `${this.baseUrl}/behavior/api/v1/unlike_behavior/`
    ...
    async loadCommentReplies(minDate = 20000000000000) {
      const url = `${this.baseUrl}/comment/api/v1/comment_reply/load`
    ...
    async handleSaveCommentReply() {
      ...
      const url = `${this.baseUrl}/comment/api/v1/comment_reply/save`
    ...
    async handleClickCommentReplyLike(commentReply) {
      ...
      const url = `${this.baseUrl}/comment/api/v1/comment_reply/like`
